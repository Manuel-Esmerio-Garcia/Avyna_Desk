...
$("#btn_Reimprimir").click(function(event) {
  ...
  if (window.TableGuiaEnvios != null)
  {
    ...
    formData.append("Guia", window.TableGuiaEnvios.childNodes[5].innerHTML);

    $.ajax({
    url: dir + 'index.php/Controller_Guias_Envio/PrintGlobalLabel',
    type: 'POST',
    processData: false,  // tell jQuery not to process the data
    contentType: false,
    timeout: 35000,
    ...
    success: function(data){
        ...
        switch(obj.code){
          case 0:
            ...
            if (window.TableGuiaEnvios.childNodes[5].innerHTML.length === 22) {
              window.open(dir + 'ESTAFETA/guiaprint.pdf', '_blank');
            }
            else if (window.TableGuiaEnvios.childNodes[5].innerHTML.length === 12) {
