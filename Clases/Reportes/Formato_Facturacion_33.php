...
if(isset($_GET['ID']))
{
  ...
  $QueryValidateFormat   = "SELECT * FROM Ventas WHERE ID = ".$ID;
  $ResultValidateFormat  = mysqli_query($conn, $QueryValidateFormat);
  while($idClientFormat = mysqli_fetch_array($ResultValidateFormat)){
    $IDValidateFormat = intval($idClientFormat['idCliente']);
  }
  ...
  if ($IDValidateFormat != 261 && $IDValidateFormat != 1967) {
    if ($RFC_Cliente != 'XAXX010101000') {
      $sqlQuery = "SELECT F.ID, F.Serie, F.Folio, F.Fecha_Timbrado as FechaInvoice, F.UUID, F.IDIntegrador, F.IDVenta AS VentaFactura, F.Status as StatusInvoice, F.CertificadoSAT, F.FechaSAT, F.SelloSAT, F.SelloCFD, F.RFC_PAC, F.FormaPago, F.MetodoPago, F.UsoCFDI, F.Comprobante, F.Moneda,F.Observaciones,F.Sello,F.Certificado, VM.ID AS IDVenta, CONCAT(CL.Nombre, ' ', CL.Apellidos) AS Distribuidor, CL.regimenFiscal,
            CL.Empresa, CL.RFC, CL.CP, CL.Calle_numero, CL.Colonia, CL.Ciudad, CL.Municipio,CL.Municipio,CL.Estado,CL.Pais ,CA.Codigo, CA.Producto, CA.UnidadMedida, CA.UnidadSAT, CA.ClaveSAT,DVM.Importe ,DVM.Cantidad,DVM.Precio_unitario,DVM.idCatalogo,VM.Descuento,VM.Subtotal,VM.Impuestos,VM.Total,VM.Total_desc,V.Descuento as Descuento_Venta, V.Total AS Total_Venta, V.Tipo_Factura
          from Ventas as V join Factura as F ON V.ID = F.IDVenta join Clientes as CL ON V.idCliente = CL.ID join Ventas_menudeo as VM ON V.ID = VM.idVenta join detalle_venta_menudeo_oferta AS DVM on VM.ID = DVM.idVenta_menudeo join Catalogo AS CA on CA.ID = DVM.idCatalogo
                where F.IDVenta = " . $ID; 
      }
      else{
      $sqlQuery = "SELECT F.ID, F.Serie, F.Folio, F.Fecha_Timbrado as FechaInvoice, F.UUID, F.IDIntegrador, F.IDVenta AS VentaFactura, F.Status as StatusInvoice, F.CertificadoSAT, F.FechaSAT, F.SelloSAT, F.SelloCFD, F.RFC_PAC, F.FormaPago, F.MetodoPago, F.UsoCFDI, F.Comprobante, F.Moneda,F.Observaciones,F.Sello,F.Certificado, VM.ID AS IDVenta, CONCAT(CL.Nombre, ' ', CL.Apellidos) AS Distribuidor, CL.regimenFiscal,
    ...
    if ($RFC_Cliente != 'XAXX010101000') {
      $sqlQuery = "SELECT F.ID, F.Serie, F.Folio, F.Fecha_Timbrado as FechaInvoice, F.UUID, F.IDIntegrador, F.IDVenta AS VentaFactura, F.Status as StatusInvoice, F.CertificadoSAT, F.FechaSAT, F.SelloSAT, F.SelloCFD, F.RFC_PAC, F.FormaPago, F.MetodoPago, F.UsoCFDI, F.Comprobante, F.Moneda,F.Observaciones,F.Sello,F.Certificado, VM.ID AS IDVenta, CONCAT(CL.Nombre, ' ', CL.Apellidos) AS Distribuidor, CL.Empresa, CL.RFC, CL.CP, CL.Calle_numero, CL.Colonia, CL.Ciudad, CL.Municipio,CL.Municipio,CL.Estado,CL.Pais ,CA.Codigo, CA.Producto, CA.UnidadMedida, CA.UnidadSAT, CA.ClaveSAT,DVM.Importe ,DVM.Cantidad,DVM.Precio_unitario,DVM.idCatalogo,VM.Descuento,VM.Subtotal,VM.Impuestos,VM.Total,VM.Total_desc,V.Descuento as Descuento_Venta, V.Total AS Total_Venta, V.Tipo_Factura
      FROM Ventas as V 
  ...
  $sql_Promociones = "SELECT DVMO.*, P.Promocion, ('XPK') AS ClaveUnidad, (01010101) AS ClaveProdServ FROM detalle_venta_menudeo_oferta AS DVMO INNER JOIN promociones AS P ON DVMO.idPromocion = P.ID WHERE DVMO.idCatalogo IS NULL AND DVMO.ID_Venta =".$ID;


  $sqlResult      = mysqli_query($conn, $sqlQuery);
  $sqlMovimientos = mysqli_query($conn, $sqlQuery);
  $sqlPromociones = mysqli_query($conn, $sql_Promociones);
  ...
  if(empty($sqlPromociones) == false)
  {
    $rowsPromo = mysqli_num_rows($sqlPromociones);
  }
  ...
  while($rowTemp = mysqli_fetch_array($sqlResult))
  {
    ...
    if ($rowTemp['Tipo_Factura'] == 1){
      ...
      if ($rowTemp['RFC'] != null && $rowTemp['RFC'] != "") {
  ...
  else if($idioma == "EN")
  {
    ...
    if ($rowsPromo > 0)
    {
      while ($Promo = mysqli_fetch_array($sqlPromociones))
      {
          $Producto               = $Promo['Promocion'];
          $UnidadSAT              = $Promo['ClaveUnidad'];
          $ClaveSAT               = $Promo['ClaveProdServ'];
          $Importe                = $Promo['Importe'];
          $Cantidad               = $Promo['Cantidad'];
          $Precio_unitario        = $Promo['Precio_unitario'];
          $Impuestos_Movimiento   = $Importe  - ($Importe / 1.16);
    ...
    if ($IVA == 0.16) {
        ...
        $Subtotal_Venta = floatval($Total_Venta) / $IVA_Valor;
    }

    $Impuestos_Venta = floatval($Total_Venta) - $Subtotal_Venta;
     ...
     $sqlResultRelacion = mysqli_query($conn, $sqlQuery4);
      ...
      if(empty($sqlResultRelacion) == false)
      {
        $rowsRelacion = mysqli_num_rows($sqlResultRelacion);
      }
