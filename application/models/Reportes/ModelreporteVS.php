...
class ModelreporteVS extends CI_Model
{
    ...
    public function exportReport($data){
        ...
        $this->db->select('inv.*,('.$data['calculo'].' * Faltante) AS Factor, IF(SUM(doc.Restantes) IS NULL,0,SUM(doc.Restantes)) AS Total_Restante, CONCAT(p.Nombre," ",p.Apellidos) AS Proveedor, IF(oc.Status = "Aceptada" OR oc.Status = "Generada", SUM(doc.Cantidad),"Sin Aceptar o Generar") AS Aceptadas_Generadas'); 
...
// ORDER BY `inv`.`Producto` DESC 
        ...
        if($data["search"]["value"] !== ''){
            ...
            $this->db->or_like('IF(SUM(doc.Restantes) IS NULL,0,SUM(doc.Restantes))', $data['search']['value']);
